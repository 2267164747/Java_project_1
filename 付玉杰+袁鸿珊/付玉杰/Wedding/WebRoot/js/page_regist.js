	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			email: {
				...
				email: $.i18n.prop("请输入对的Email")
			},
			password: {
				required: $.i18n.prop("请注意密码格式"),
				minlength: jQuery.format($.i18n.prop("注意密码格式")),
				maxlength: jQuery.format($.i18n.prop("请注意密码格式"))
			},
			passwordAgain: {
				required: $.i18n.prop("请再次输入密码"),
				minlength: jQuery.format($.i18n.prop("请注意密码格式")),
				maxlength: jQuery.format($.i18n.prop("请输入密码最大值")),
				equalTo: jQuery.format($.i18n.prop("请输入相同的密码"))
			},
			contact: {
			...
			tel: {
				...
				minlength: jQuery.format($.i18n.prop("请输入11位数字")),
				maxlength: jQuery.format($.i18n.prop("请输入11位数字")),
...
function regist(validate){	
	...
	if(validate.form()){
		if($("#checkBox").attr("checked")){
			...
			//勾选隐私政策和服务条款
			$(".login-error").show();
			$(".login-error").html($.i18n.prop("请认真阅读并同意条款"));
		}
	}
}
